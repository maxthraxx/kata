---
name: kata-querying-intelligence
description: Use this skill when querying project intelligence, searching context, or finding information. Triggers include "query intel", "search context", and "find information".
---
...
**Requires:** `.planning/intel/graph.db` (created by `/kata:analyze-codebase` with entity generation)
...
**For dependents:**
```bash
echo '{"action":"query","type":"dependents","target":"'$ENTITY_ID'","limit":20}' | node hooks/kata-intel-index.js
```

**For hotspots:**
```bash
echo '{"action":"query","type":"hotspots","limit":5}' | node hooks/kata-intel-index.js
```
...
Run /kata:analyze-codebase first to build the dependency graph.
```
...
- /kata:analyze-codebase to rebuild the index
