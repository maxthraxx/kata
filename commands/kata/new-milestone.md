...
1. **Verify project exists:**
   ```bash
   [ -f .planning/PROJECT.md ] || { echo "ERROR: No PROJECT.md. Run /kata:new-project first."; exit 1; }
   ```
...
2. **Check for active milestone (ROADMAP.md exists):**
   ...
   **If ACTIVE_MILESTONE:**
   ...
   - options:
     - "Complete current first" — Run /kata:complete-milestone
     - "Continue anyway" — Start new milestone (will archive current)

   If "Complete current first": Exit with routing to `/kata:complete-milestone`
...
Spawn 4 parallel kata-project-researcher agents with context:
