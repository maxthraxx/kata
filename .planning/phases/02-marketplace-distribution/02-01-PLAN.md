...
files_modified:
  ...
  - (external) gannonh/gannonh-plugins/.claude-plugin/marketplace.json
autonomous: true

must_haves:
  truths:
    - "Kata plugin version is 0.1.9 in plugin.json"
    - "gannonh-plugins GitHub repository exists"
    - "marketplace.json contains Kata plugin entry"
  artifacts:
    - path: ".claude-plugin/plugin.json"
      provides: "Plugin manifest with v0.1.9"
      contains: '"version": "0.1.9"'
    - path: "(external) gannonh/gannonh-plugins/.claude-plugin/marketplace.json"
      provides: "Marketplace catalog with Kata entry"
      contains: '"name": "kata"'
  key_links:
    - from: "gannonh-plugins/marketplace.json"
      to: "gannonh/kata"
...
Purpose: Enable users to install Kata via `/plugin install kata@gannonh-plugins`
Output: gannonh-plugins repository with marketplace.json pointing to gannonh/kata
</objective>
...
- marketplace.json format:
  ...
  ```
- Users add via: `/plugin marketplace add gannonh/gannonh-plugins`
- Users install via: `/plugin install kata@gannonh-plugins`
</context>
...
<task type="auto">
  <name>Task 2: Create gannonh-plugins marketplace repository</name>
  <files>(external) gannonh/gannonh-plugins repo</files>
  <action>
Create the gannonh-plugins GitHub repository:

1. Create a temporary directory for the marketplace repo:
   ```bash
   mkdir -p /tmp/gannonh-plugins/.claude-plugin
   ```
...
4. Initialize git repo and push to GitHub:
   ```bash
   cd /tmp/gannonh-plugins
   git init
   git add .
   git commit -m "Initial marketplace with Kata plugin"
   gh repo create gannonh-plugins --public --source=. --push --description "Personal plugin marketplace for Claude Code extensions"
   ```
  </action>
  <verify>gh repo view gannonh/gannonh-plugins --json name,description</verify>
  <done>gannonh-plugins repository exists on GitHub with marketplace.json containing Kata entry</done>
</task>

<task type="auto">
  <name>Task 3: Validate marketplace locally</name>
  <files>(external) gannonh/gannonh-plugins</files>
  <action>
Clone the marketplace repo and validate it:

```bash
cd /tmp
rm -rf gannonh-plugins-validate
git clone https://github.com/gannonh/gannonh-plugins.git gannonh-plugins-validate
cd gannonh-plugins-validate
...
2. `gh repo view gannonh/gannonh-plugins` shows repo exists
3. `curl -s https://raw.githubusercontent.com/gannonh/gannonh-plugins/main/.claude-plugin/marketplace.json | jq '.plugins[0].name'` returns "kata"
</verification>
