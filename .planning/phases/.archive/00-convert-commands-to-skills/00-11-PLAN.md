...
Purpose: UAT found that kata-verification does automated must_haves verification but never creates UAT.md or presents interactive tests to users. The skill needs to integrate the UAT workflow from phase-verify.md.

Output: Updated SKILL.md with complete UAT workflow, plus new reference document for UAT protocol.
</objective>

<execution_context>
@./.claude/kata/workflows/phase-verify.md
...
<task type="auto">
  ...
  <action>
Review and enhance the EXISTING uat-protocol.md (234 lines) to ensure it includes all UAT execution guidance from phase-verify.md.
...
Check if these elements from phase-verify.md are present; add if missing:
...
2. Add new Step 5: UAT Workflow (model after phase-verify.md steps):
```markdown
### Step 5: UAT Workflow
