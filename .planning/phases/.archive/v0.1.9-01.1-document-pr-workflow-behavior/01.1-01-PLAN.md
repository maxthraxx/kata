...
must_haves:
  ...
  key_links:
    - from: "kata/references/planning-config.md"
      ...
      pattern: "phase-execute|milestone-complete|progress"
---

<objective>
Document complete pr_workflow config behavior in planning-config.md

Purpose: Developers need clear reference documentation for how pr_workflow: true changes Kata behavior — branch naming, PR lifecycle, release timing, and command integration points.
...
- Version bump timing: User bumps before /kata:milestone-complete
...
If any gaps found, fix them.
  ...
  <verify>grep -E "branch naming|PR.*phase|release.*milestone|workflow timing|phase-execute.*branch" kata/references/planning-config.md returns matches for all criteria</verify>
...
grep -c "phase-execute" kata/references/planning-config.md # Should be >= 2
