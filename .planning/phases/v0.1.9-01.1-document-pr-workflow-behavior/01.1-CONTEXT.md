...
- **Version bump timing:** User bumps manually before `/kata:complete-milestone`, workflow verifies it's done
...
|---------|------------------|------------------------|
| new-project | Asks about pr_workflow | Offers GitHub Actions scaffold |
| settings | Allows toggling | Same |
| execute-phase | Commits to main | **Create branch, open draft PR** (NEW) |
| complete-milestone | Creates local tag | Skips tag, offers PR creation |
...
- execute-phase and progress need new integration (marked as NEW above) — document what SHOULD happen, implementation is separate phase
...
- **Implement execute-phase branch/PR creation** — The documentation describes what SHOULD happen; actual implementation is a separate phase.
